...
class SignalTracker:
    ...
    async def get_prior_signals_summary(
        self, market_ids: list[str],
    ) -> dict[str, tuple[int, float]]:
        """Get (count, first_edge) for each market_id that has prior signals.

        Args:
            market_ids: List of market IDs to check.

        Returns:
            Dict mapping market_id → (signal_count, first_edge).
            Missing keys mean the market has no prior signals.
        """
        if not market_ids:
            return {}
        await self._ensure_db()
        if self._use_pg:
            return await self._prior_signals_pg(market_ids)
        return await self._prior_signals_sqlite(market_ids)

    async def _prior_signals_pg(
        self, market_ids: list[str],
    ) -> dict[str, tuple[int, float]]:
        pool = await self._get_pool()
        async with pool.acquire() as conn:
            rows = await conn.fetch(
                """SELECT market_id, COUNT(*) AS cnt,
                          (SELECT edge FROM signals s2
                           WHERE s2.market_id = s1.market_id
                           ORDER BY s2.id LIMIT 1) AS first_edge
                   FROM signals s1
            ...
            return {
                row["market_id"]: (row["cnt"], row["first_edge"])
                for row in rows
            }

    async def _prior_signals_sqlite(
        self, market_ids: list[str],
    ) -> dict[str, tuple[int, float]]:
        placeholders = ",".join("?" for _ in market_ids)
        async with aiosqlite.connect(str(self._db_path)) as db:
            cursor = await db.execute(
                f"""SELECT market_id, COUNT(*) AS cnt,
                           (SELECT edge FROM signals s2
                            WHERE s2.market_id = s1.market_id
                            ORDER BY s2.id LIMIT 1) AS first_edge
                    FROM signals s1
    ...
    async def get_resolved_signals(
        ...
        agg_func = "MIN" if first_signal else "MAX"

        # Deduplicate: keep one signal per market_id (first or latest).
        base_query = f"""
            ...
            FROM signals s
            INNER JOIN (
                SELECT market_id, {agg_func}(id) AS max_id
                FROM signals
                WHERE outcome IS NOT NULL
                GROUP BY market_id
