...
def format_telegram_signal(
    ...
    if signal_number is not None and signal_number > 1:
        first_edge_str = f"{first_edge:+.1%}" if first_edge is not None else "?"
        header = f"\U0001f504 *UPDATE — signal #{signal_number}* (first edge: {first_edge_str})"
    else:
        header = "\U0001f195 *NEW SIGNAL*"
