...
class TestFormatTelegramSignal:
    def test_new_signal_header_by_default(self):
        signal = _make_signal()
        text = format_telegram_signal(signal)
        assert "*NEW SIGNAL*" in text

    def test_update_header_when_signal_number_gt_1(self):
        signal = _make_signal()
        text = format_telegram_signal(signal, signal_number=5, first_edge=0.15)
        assert "*UPDATE" in text
        assert "signal #5" in text
        assert "+15.0%" in text

    def test_new_header_when_signal_number_is_1(self):
        signal = _make_signal()
        text = format_telegram_signal(signal, signal_number=1)
        assert "*NEW SIGNAL*" in text
...
class TestTelegramNotifier:
    ...
    async def test_notify_sends_high_edge_alerts_and_summary(self):
        ...
        mock_tracker.get_prior_signals_summary = AsyncMock(return_value={})
        mock_tracker.close = AsyncMock()

        with patch("kalshi_weather.notifications.telegram.SignalTracker", return_value=mock_tracker):
            await notifier.notify(signals, high_edge_threshold=0.10)

        # All 3 signals get individual alerts
    ...
    async def test_notify_signal_formats_and_sends(self):
        ...
        assert "*NEW SIGNAL*" in sent_text
