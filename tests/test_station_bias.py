...
from kalshi_weather.forecasting.temperature import (
    TemperatureModel,
...
def test_load_biases_from_json(tmp_path):
    ...
    with patch("kalshi_weather.calibration.station_bias.get_settings") as mock_settings:
...
def test_load_fallback(tmp_path):
    ...
    with patch("kalshi_weather.calibration.station_bias.get_settings") as mock_settings:
        ...
        # Bundled defaults are empty until calibration is run
        assert len(biases) == 0
...
def test_get_station_bias_disabled():
    ...
    with patch("kalshi_weather.calibration.station_bias.get_settings") as mock_settings:
        ...
        result = get_station_bias("KATL", "max")
        assert result == 0.0
...
def test_apply_shifts_mean():
    ...
    with patch("kalshi_weather.forecasting.temperature.get_station_bias_for_condition", return_value=1.5):
...
def test_apply_preserves_spread():
    ...
    with patch("kalshi_weather.forecasting.temperature.get_station_bias_for_condition", return_value=2.0):
...
def test_apply_no_correction_when_zero():
    ...
    with patch("kalshi_weather.forecasting.temperature.get_station_bias_for_condition", return_value=0.005):
        corrected, detail = _apply_station_bias_correction(members, "KTEST1", "max")
...
async def test_end_to_end_probability_shift():
    ...
    with patch("kalshi_weather.forecasting.temperature.get_station_bias_for_condition", return_value=0.0):
...
def test_apply_with_cloud_cover():
    ...
    with patch("kalshi_weather.forecasting.temperature.get_station_bias_for_condition", return_value=3.0) as mock_fn:
...
def test_try_load_from_db_returns_parsed(tmp_path):
    ...
    with patch("kalshi_weather.calibration.station_bias.get_settings") as mock_settings, \
         patch("kalshi_weather.signals.tracker.SignalTracker", return_value=mock_tracker):
...
def test_try_load_from_db_handles_error():
    ...
    with patch("kalshi_weather.calibration.station_bias.get_settings") as mock_settings, \
         patch("kalshi_weather.signals.tracker.SignalTracker", side_effect=Exception("connection error")):
        mock_settings.return_value.database_url = "postgresql://localhost/test"
        result = _try_load_from_db()
...
def test_save_biases_skips_db_when_no_url(tmp_path):
    ...
    bias_file = tmp_path / "biases.json"
    with patch("kalshi_weather.calibration.station_bias.get_settings") as mock_settings, \
         patch("kalshi_weather.signals.tracker.SignalTracker") as mock_tracker_cls:
        mock_settings.return_value.station_bias_enabled = True
        mock_settings.return_value.station_bias_path = bias_file
        mock_settings.return_value.database_url = ""
