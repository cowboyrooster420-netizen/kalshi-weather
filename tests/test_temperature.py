...
from kalshi_weather.forecasting.temperature import TemperatureModel, _compute_dynamic_weights, _hrrr_blend_weight
from kalshi_weather.markets.models import Comparison, MarketParams, MarketType
from kalshi_weather.weather.models import EnsembleForecast, HRRRForecast
...
async def test_temperature_above_unlikely(ecmwf_forecast, gfs_forecast, now):
    ...
    assert estimate.probability <= 0.12  # Clamped to probability floor
...
async def test_temperature_probability_bounds(ecmwf_forecast, gfs_forecast, now):
    ...
    assert estimate.probability >= 0.12   # probability_floor
    assert estimate.probability <= 0.88   # 1 - probability_floor
...
async def test_daily_min_aggregation(now):
    ...
    # Both calibrated values may be clamped to the floor, so check raw_probability
    # which is not subject to the probability floor.
    assert est_daily.raw_probability > est_single.raw_probability
...
async def test_hrrr_ignored_beyond_18h(now):
    ...
    # Should be identical — HRRR ignored beyond 18h (tiny float diff from clock drift)
    assert abs(est_with_hrrr.probability - est_no_hrrr.probability) < 1e-6
...
# --- Probability floor/ceiling tests ---


@pytest.mark.asyncio
async def test_probability_floor_clamps_low_probability(now):
    """Very low ensemble probability should be clamped to the floor (0.12)."""
    np.random.seed(42)
    times = [now + timedelta(hours=i) for i in range(48)]
    n = len(times)

    # Ensemble mean ~35C, threshold 65C — essentially impossible
    ecmwf = EnsembleForecast(
        source="ecmwf", lat=33.45, lon=-112.07,
        times=times,
        temperature_2m=np.random.normal(35, 2, (n, 51)),
        precipitation=np.zeros((n, 51)),
    )
    ...
    params = MarketParams(
        ...
        location="Phoenix, AZ",
        lat_lon=(33.45, -112.07),
        threshold=65.0,  # 65C — way above ensemble
        comparison=Comparison.ABOVE,
        unit="C",
        target_date=now + timedelta(hours=24),
    )
    ...
    # Should be clamped to exactly the floor
    assert estimate.probability == pytest.approx(0.12, abs=1e-9)


@pytest.mark.asyncio
async def test_probability_ceiling_clamps_high_probability(now):
    """Very high ensemble probability should be clamped to the ceiling (0.88)."""
    np.random.seed(42)
    times = [now + timedelta(hours=i) for i in range(48)]
    n = len(times)

    # Ensemble mean ~35C, threshold 0C — virtually certain to exceed
    ecmwf = EnsembleForecast(
        source="ecmwf", lat=33.45, lon=-112.07,
        times=times,
        temperature_2m=np.random.normal(35, 2, (n, 51)),
    ...
    params = MarketParams(
        ...
        location="Phoenix, AZ",
        lat_lon=(33.45, -112.07),
        threshold=0.0,  # 0C — way below ensemble mean
        comparison=Comparison.ABOVE,
        unit="C",
        target_date=now + timedelta(hours=24),
    )
    ...
    # Should be clamped to exactly the ceiling
    assert estimate.probability == pytest.approx(0.88, abs=1e-9)


@pytest.mark.asyncio
async def test_raw_probability_not_clamped_by_floor(now):
    """Raw probability should still use 0.001-0.999 bounds, not the floor."""
    np.random.seed(42)
    times = [now + timedelta(hours=i) for i in range(48)]
    n = len(times)

    # Ensemble mean ~35C, threshold 65C — raw prob should be near 0
    ecmwf = EnsembleForecast(
        source="ecmwf", lat=33.45, lon=-112.07,
        times=times,
        temperature_2m=np.random.normal(35, 2, (n, 51)),
    ...
    # Raw probability should be much lower than the floor
    assert estimate.raw_probability < 0.12
    # But still within [0.001, 0.999]
    assert estimate.raw_probability >= 0.001


@pytest.mark.asyncio
async def test_probability_floor_is_config_driven(now, monkeypatch):
    """Changing probability_floor in config should change the clamp."""
    np.random.seed(42)
    times = [now + timedelta(hours=i) for i in range(48)]
    n = len(times)
    ...
    # Override the floor to 0.20
    import kalshi_weather.config as config_mod
    original_settings = config_mod._settings_cache
    try:
        config_mod._settings_cache = config_mod.Settings(probability_floor=0.20)
        model = TemperatureModel()
        estimate = await model.estimate(params, gfs=None, ecmwf=ecmwf, noaa=None)
        assert estimate.probability == pytest.approx(0.20, abs=1e-9)
    finally:
        config_mod._settings_cache = original_settings
